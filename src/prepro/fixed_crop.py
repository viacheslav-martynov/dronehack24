"""
This script implements a crop around objects which size is less than max_pixel_size
with crop_sise.
...
def crop_objects(image, image_labels, object_classes, objects_for_crop, crop_size, random=True):
    ...
    for object_for_crop in objects_for_crop:
        ...
        if random:
            ...
            sigma = np.random.uniform(0.25,0.6)
...
def main(base_dir: str, save_dir: str, max_pixel_size: int = 128, crop_size: int = 640, log_file: str = "empty_images.txt"):
    ...
    with open(log_file, 'w') as log:
        for image_path in tqdm(image_paths):
            ...
            objects_for_crop = get_objects_for_crop(boxes_pixel_xyxy, max_pixel_size)
            crops, crops_new_labels = crop_objects(image, boxes_pixel_xyxy, object_classes, objects_for_crop, crop_size)
            if not crops:
